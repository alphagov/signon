...
class ApplicationTableHelperTest < ActionView::TestCase
  ...
  context "#update_permissions_link" do
    ...
    [
      { role: :govuk_admin?, app: :has_non_signin_permissions_grantable_from_ui? },
      { role: :publishing_manager?, app: :has_delegatable_non_signin_permissions_grantable_from_ui? },
    ].each do |context_methods|
      context "when `current_user.#{context_methods[:role]}` is true" do
        setup { @current_user.expects(context_methods[:role]).returns(true) }

        context "when `application.#{context_methods[:app]}` is true" do
          setup { @application.expects(context_methods[:app]).returns(true) }
        ...
        context "when `application.#{context_methods[:app]}` is false" do
          should "return an empty string" do
            @application.expects(context_methods[:app]).returns(false)
